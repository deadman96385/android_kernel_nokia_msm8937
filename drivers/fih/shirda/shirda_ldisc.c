/* shirda_ldisc.c (Infrared driver module)
 *
...
#ifdef SHIRDA_DEBUG
#undef SHIRDA_LDISC_DEBUG
#endif

#ifdef	SHIRDA_LDISC_DEBUG
#undef SHIRDA_DEBUG_SEND_DELAY
#undef SHIRDA_DEBUG_TIMESTAMP
#define	SHIRDA_DEBUG_ENTRY_POINT

#ifdef SHIRDA_TEST_GPIO_DEBUG
#define SHIRDA_RX_DEBUG_GPIO
#define SHIRDA_TX_DEBUG_GPIO

#ifdef SHIRDA_RX_DEBUG_GPIO
#define SHIRDA_RX_DEBUG_GPIO_PIN	SHIRDA_GPIO_TEST1
#endif
#ifdef SHIRDA_TX_DEBUG_GPIO
#define SHIRDA_TX_DEBUG_GPIO_PIN	SHIRDA_GPIO_TEST1
#endif
#endif

#endif

#undef SHIRDA_SEND_ERR_ENABLE
...
#ifdef SHIRDA_DEBUG_SEND_DELAY

#define DEBUG
#include <linux/fs.h>

int shirda_ldisc_txready_time_value = 0;
module_param(shirda_ldisc_txready_time_value, int,
						S_IRUGO | S_IWUSR | S_IWGRP);

int shirda_ldisc_txempty_time_value = 0;
module_param(shirda_ldisc_txempty_time_value, int,
						S_IRUGO | S_IWUSR | S_IWGRP);

#define SHIRDA_TXREADY_WAIT  (shirda_ldisc_txready_time_value ? \
				shirda_ldisc_txready_time_value : 5000)

#define SHIRDA_TXEMPTY_WAIT (shirda_ldisc_txempty_time_value ? \
				shirda_ldisc_txempty_time_value : 5000)

enum shirda_ldisc_delay_count_enum {
	SHIRDA_LDISC_COUNT_TXREADY_LATE = 0,
	SHIRDA_LDISC_COUNT_TXREADY_TOO_LATE,
	SHIRDA_LDISC_COUNT_TXREADY_ERR,
	SHIRDA_LDISC_COUNT_TXEMPTY_LATE,
	SHIRDA_LDISC_COUNT_TXEMPTY_TOO_LATE,
	SHIRDA_LDISC_COUNT_TXEMPTY_ERR,
	SHIRDA_LDISC_COUNT_MAX,
};

static char *shirda_ldisc_delay_count_name[] = {
	"TXREADY_LATE    ",
	"TXREADY_TOO_LATE",
	"TXREADY_ERROR   ",
	"TXEMPTY_LATE    ",
...
static long shirda_ldisc_delay_count[SHIRDA_LDISC_COUNT_MAX];
static long shirda_ldisc_delay_count_sum[SHIRDA_LDISC_COUNT_MAX];

enum shirda_ldisc_delay_time_enum {
	SHIRDA_LDISC_TIME_TXREADY = 0,
	SHIRDA_LDISC_TIME_TXEMPTY,
	SHIRDA_LDISC_TIME_MAX,
};

static char *shirda_ldisc_delay_time_name[] = {
	"TXREADY DELAY TIME",
	"TXEMPTY DELAY TIME",
	"MAX",
};

static long shirda_ldisc_delay_time[SHIRDA_LDISC_TIME_MAX];
static long shirda_ldisc_delay_time_sum[SHIRDA_LDISC_TIME_MAX];

static void shirda_ldisc_delay_count_up(int a)
{
	shirda_ldisc_delay_count[a]++;
	return;
}

static void shirda_ldisc_delay_count_up_nz(int a, int b)
{
	if (b != 0) shirda_ldisc_delay_count[a]++;
	return;
}

static void shirda_ldisc_delay_time_set(int a, long b)
{
	if (shirda_ldisc_delay_time[a] < b) {
		shirda_ldisc_delay_time[a] = b;
	}
	return;
}

static void shirda_ldisc_delay_time_set_nz(int a, long b, int c)
{
	if (c != 0) {
		if (shirda_ldisc_delay_time[a] < b) {
			shirda_ldisc_delay_time[a] = b;
...
static void shirda_ldisc_delay_clear(void)
{
	int i;

	printk("shirda ldisc open\n");
	for (i = 0; i < SHIRDA_LDISC_COUNT_MAX; i++) {
		shirda_ldisc_delay_count[i] = 0;
	}
	for (i = 0; i < SHIRDA_LDISC_TIME_MAX; i++) {
		shirda_ldisc_delay_time[i] = 0;
	}
	return;
}

static void shirda_ldisc_delay_display(void)
{
	int i;

	for (i = 0; i < SHIRDA_LDISC_COUNT_MAX; i++) {
		shirda_ldisc_delay_count_sum[i] += shirda_ldisc_delay_count[i];
		printk("SHIRDA LDISC COUNT %s  : %04ld %04ld\n",
			shirda_ldisc_delay_count_name[i],
			shirda_ldisc_delay_count_sum[i],
			shirda_ldisc_delay_count[i]);
	}
	for (i = 0; i < SHIRDA_LDISC_TIME_MAX; i++) {
		if (shirda_ldisc_delay_time_sum[i] <
						 shirda_ldisc_delay_time[i]) {
			shirda_ldisc_delay_time_sum[i] =
						shirda_ldisc_delay_time[i];
		}
		printk("SHIRDA LDISC TIME %s : %06ld %06ld\n",
			shirda_ldisc_delay_time_name[i],
			shirda_ldisc_delay_time_sum[i],
			shirda_ldisc_delay_time[i]);
	}
	printk("shirda ldisc close\n");
	return;
}
...
#ifdef SHIRDA_LDISC_DEBUG
void shirda_log_buf(const unsigned char* buf, int nr)
{
	#define CHAR_IN_A_LINE 32
	unsigned char c[CHAR_IN_A_LINE];
	int n=0, i=0;
	while (n<nr){
		for (i=0;i<CHAR_IN_A_LINE;i++){
			if (n+i<nr){
				c[i] = buf[n+i];
			} else {
				c[i] = 0x00;
...
#ifdef SHIRDA_DEBUG_TIMESTAMP
	now = ktime_get();
	IRDALOG_INFO("mtt=%lu,last_rx=%llu,now=%llu",
							mtt,
							mtt_kp->last_rx.tv64,
							now.tv64);
#endif
...
#ifdef SHIRDA_DEBUG_SEND_DELAY
		shirda_ldisc_delay_count_up(SHIRDA_LDISC_COUNT_TXREADY_ERR);
		shirda_ldisc_delay_time_set(SHIRDA_LDISC_TIME_TXREADY,
			ktime_to_nsec(ktime_sub(now, ktime_get())) / 1000);
#endif
#ifdef SHIRDA_SEND_ERR_ENABLE
		ret = -EIO;
#endif
	} else {
...
#ifdef SHIRDA_DEBUG_SEND_DELAY
			shirda_ldisc_delay_count_up_nz(
				SHIRDA_LDISC_COUNT_TXREADY_LATE,
				repeat - 100);
			shirda_ldisc_delay_time_set_nz(
				SHIRDA_LDISC_TIME_TXREADY,
				usec, repeat - 100);
#endif
		}
	}
								...
								ktime_t now)
{
...
#ifdef SHIRDA_TX_DEBUG_GPIO
	gpio_set_value(SHIRDA_TX_DEBUG_GPIO_PIN, 0);
#endif
	...
	if (repeat <= 0) {
		...
		ret = -EIO;
#ifdef SHIRDA_DEBUG_SEND_DELAY
		shirda_ldisc_delay_count_up(SHIRDA_LDISC_COUNT_TXEMPTY_ERR);
		shirda_ldisc_delay_time_set(SHIRDA_LDISC_TIME_TXEMPTY,
			ktime_to_nsec(ktime_sub(now, ktime_get())) / 1000);
#endif
	} else {
		long usec = ktime_to_nsec(ktime_sub(ktime_get(), now)) / 1000;
...
#ifdef	SHIRDA_TTY_WITH_ENTITY_OF_TERMIOS
...
#ifdef SHIRDA_RX_DEBUG_GPIO
	gpio_set_value(SHIRDA_RX_DEBUG_GPIO_PIN, 1);
	IRDALOG_INFO("Rx debug output gpio %d\n", SHIRDA_RX_DEBUG_GPIO_PIN);
#endif
#ifdef SHIRDA_TX_DEBUG_GPIO
	gpio_set_value(SHIRDA_TX_DEBUG_GPIO_PIN, 1);
	IRDALOG_INFO("Tx debug output gpio %d\n", SHIRDA_TX_DEBUG_GPIO_PIN);
#endif
...
#ifdef SHIRDA_DEBUG_SEND_DELAY
	shirda_ldisc_delay_clear();
#endif

#ifdef SHIRDA_RX_DEBUG_GPIO
	gpio_set_value(SHIRDA_RX_DEBUG_GPIO_PIN, 0);
#endif
#ifdef SHIRDA_TX_DEBUG_GPIO
	gpio_set_value(SHIRDA_TX_DEBUG_GPIO_PIN, 0);
#endif
...
#ifdef SHIRDA_LDISC_DEBUG
	shirda_log_buf(admin->tx_frame,admin->nr_tx_frame);
#endif
...
static int __init n_shirda_init(void)
{
	...
	for (i=0; i < SHIRDA_LDISC_COUNT_MAX; i++)
		shirda_ldisc_delay_count_sum[i]=0;

	for (i=0; i < SHIRDA_LDISC_TIME_MAX; i++)
		shirda_ldisc_delay_time_sum[i]=0;
