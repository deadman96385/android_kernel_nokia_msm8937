...
#include "fih_simstate.h"

#define FIH_PROC_DIR   "AllHWList"
#define FIH_PROC_SIMSTATE_PATH  "AllHWList/simstate"
#define FIH_PROC_SIZE  FIH_SIM_SIZE

static char fih_proc_data[FIH_PROC_SIZE] = "0";

/*simstate*/
void fih_simstate_setup(char *info)
{
...
static int simstate_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_proc_read_show, NULL);
};

static struct file_operations simstate_file_ops = {
	.owner   = THIS_MODULE,
	.open    = simstate_proc_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
{
	if (proc_create(FIH_PROC_SIMSTATE_PATH, 0, NULL, &simstate_file_ops) == NULL) {
		proc_mkdir(FIH_PROC_DIR, NULL);
		if (proc_create(FIH_PROC_SIMSTATE_PATH, 0, NULL, &simstate_file_ops) == NULL) {
			pr_err("fail to create proc/%s\n", FIH_PROC_SIMSTATE_PATH);
			return (1);
		}
	}
	return (0);
...
{
	remove_proc_entry(FIH_PROC_SIMSTATE_PATH, NULL);
}
