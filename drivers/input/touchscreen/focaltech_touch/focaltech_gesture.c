...
#define FTS_GESTRUE_POINTS                      250//255
...
*****************************************************************************/
void fts_touch_gesture_write(int enable)
{
...
int fts_touch_gesture_read(void)
{
    struct input_dev *input_dev = fts_data->input_dev;
    ...
    FTS_INFO("%s, gesture %s, reg=%d\n", __func__, fts_gesture_data.mode?"ON":"OFF", val);
    return fts_gesture_data.mode;
}

void fts_touch_gesture_available_write(long enable)
{
    struct input_dev *input_dev = fts_data->input_dev;
    mutex_lock(&input_dev->mutex);
    if (enable >= 1) {
...
int fts_touch_gesture_available_read(void)
{
    struct input_dev *input_dev = fts_data->input_dev;
    struct i2c_client *client = fts_data->client;
    u8 val;
