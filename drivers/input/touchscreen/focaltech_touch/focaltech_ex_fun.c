...
static struct rwreg_operation_t dsirw_op;

/*****************************************************************************
* Global variable or extern global variabls/functions
*****************************************************************************/
void fts_touch_tpfwver_read(char *buf)
{
...
#endif
    if (fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &vendor_id) < 0) {
        FTS_ERROR("Get vendor id fail");
        sprintf(buf, "%s", "error");
    } else {
        if (vendor_id == FTS_VENDOR_ID) {
            FTS_INFO("Self test for vendor 1");
            sprintf(buf, "%s", "HLT");
        } else if (vendor_id == FTS_VENDOR_ID2) {
            FTS_INFO("Self test for vendor 2");
            sprintf(buf, "%s", "TXD");
        } else {
            FTS_ERROR("Invalid vendor id, self test for vendor 1");
            sprintf(buf, "%s", "invalid");
        }
    }

    if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fwver) < 0) {
        FTS_ERROR("I2c transfer error!\n");
        sprintf(buf, "%s-%s", buf, "error");
    } else {
        if ((fwver == 0xFF) || (fwver == 0x00))
            sprintf(buf, "%s-%s", buf, "error");
        else
            sprintf(buf, "%s-%02x", buf, fwver);
    }
#if FTS_ESDCHECK_EN
...
void fts_touch_vendor_read(char *buf)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    struct i2c_client *client = ts_data->client;
    ...
    if (vendor_id == FTS_VENDOR_ID) {
        sprintf(vendor, "%s", "HLT");
    } else if (vendor_id == FTS_VENDOR_ID2) {
        sprintf(vendor, "%s", "TXD");
    } else {
        FTS_ERROR("Invalid vendor id, default as HLT\n");
        sprintf(vendor, "%s", "Unknown");
    }

    sprintf(buf, "%s, 0x%x%s\n", "Focaltech", vendor_id, vendor);
    return;
}
 ...
 * [5-6]...[n-(n+1)]: data, hex
 */
static int fts_parse_buf(const char *buf, size_t cmd_len, struct rwreg_operation_t *rw_op)
{
    int length;
    int i;
    char *tmpbuf;
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (2 == cmd_length) {
        ...
        rw_op.len = fts_parse_buf(buf, cmd_length, &rw_op);
    }
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_tpmipirw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    int i;
    struct input_dev *input_dev = fts_data->input_dev;
    ...
    if (dsirw_op.len < 0) {
        ...
        if (RWREG_OP_READ == dsirw_op.type) {
            length = dsirw_op.len - 1;
            length = (length < 0) ? 0 : length;

            count = snprintf(buf, PAGE_SIZE, "Mipi read Reg: [%02X]-[%02X]\n", dsirw_op.reg, dsirw_op.reg + length);
            count += snprintf(buf + count, PAGE_SIZE, "Result: ");
            if (dsirw_op.res) {
                count += snprintf(buf + count, PAGE_SIZE, "failed, ret: %d\n", dsirw_op.res);
            } else {
                if (dsirw_op.opbuf) {
                    for (i = 0; i < dsirw_op.len; i++) {
                        count += snprintf(buf + count, PAGE_SIZE, "%02X ", dsirw_op.opbuf[i]);
                    }
                    count += snprintf(buf + count, PAGE_SIZE, "\n");
                }
            }
        } else {
            length = dsirw_op.len - 2;
            length = (length < 0) ? 0 : length;
            count = snprintf(buf, PAGE_SIZE, "Mipi write Reg: [%02X]-[%02X]\n", dsirw_op.reg, dsirw_op.reg + length);
            count += snprintf(buf + count, PAGE_SIZE, "Write Data: ");
            if (dsirw_op.opbuf) {
                for (i = 1; i < dsirw_op.len; i++) {
                    count += snprintf(buf + count, PAGE_SIZE, "%02X ", dsirw_op.opbuf[i]);
                }
                count += snprintf(buf + count, PAGE_SIZE, "\n");
            }
...
* Return: char count
***********************************************************************/
static ssize_t fts_tpmipirw_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input_dev = fts_data->input_dev;
	ssize_t cmd_length = 0;
	...
	FTS_DEBUG("mipi cmd len: %d, buf: %s", (int)cmd_length, buf);

	if (cmd_length < 5) {
		FTS_ERROR("Invalid Mipi cmd buffer");
		mutex_unlock(&input_dev->mutex);
		return -EINVAL;
	} else {
		dsirw_op.len = fts_parse_buf(buf, cmd_length, &dsirw_op);
	}
...
#endif
	if (dsirw_op.len < 0) {
		FTS_ERROR("Mipi cmd buffer error!");
	} else {
		if (RWREG_OP_READ == dsirw_op.type) {
			char reg;
			reg = dsirw_op.reg & 0xFF;

			dsirw_op.res = fts_dsi_read_regs(reg, dsirw_op.opbuf, dsirw_op.len);
			if (dsirw_op.res < 0) {
				FTS_ERROR("Mipi could not read 0x%02x", dsirw_op.reg);
			} else {
				FTS_INFO("Mipi read 0x%02x, %d bytes successful", dsirw_op.reg, dsirw_op.len);
				dsirw_op.res = 0;
			}
		} else {
			dsirw_op.res = fts_dsi_write_regs(dsirw_op.opbuf[0], dsirw_op.opbuf+1, dsirw_op.len-1);
			if (dsirw_op.res < 0) {
				FTS_ERROR("Mipi could not write 0x%02x", dsirw_op.reg);

			} else {
				FTS_INFO("Mipi Write 0x%02x, %d bytes successful", dsirw_op.reg, dsirw_op.len-1);
				dsirw_op.res = 0;
			}
		}
	}
...
static DEVICE_ATTR(fts_mipi_rw, S_IRUGO | S_IWUSR, fts_tpmipirw_show, fts_tpmipirw_store);
