...
static const struct fg_batt_profile bqfs_image[] = {
	{ bqfs_scud, ARRAY_SIZE(bqfs_scud), 2},
	{ bqfs_mcnair, ARRAY_SIZE(bqfs_mcnair), 2},
};
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		//fih_check_temp_capacity_work(&bq->check_temp_capacity_work.work);
		//if(bq->japan_skuid){
		//	val->intval = bq->batt_report_japan_soc;
		//}else{
			val->intval = bq->batt_report_soc;
		//}
...
//<FIH,panyu add for temperature changed {{
/*
#define TEMP_DETA 10
static void fih_temp_check(struct bq_fg_chip *bq)
{
	static int pre_temp = 0;

	pr_debug("temp_now=%d, temp_pre=%d\n", bq->batt_temp - 2730,pre_temp - 2730);
...
static void fih_dump_info_workfunc(struct work_struct *work)
{
	...
	if (bq->batt_present){
		...
		pr_info("RSOC:%d report(%d), Volt:%d, Current:%d, Temperature:%d\n",
			bq->batt_soc,bq->batt_report_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp - 2730);
	}else{
...
static irqreturn_t fg_irq_thread(int irq, void *dev_id)
{
	...
	*/
	...
	if (bq->batt_present) {
		...
		pr_info("RSOC:%d,report_soc:%d, Volt:%d, Current:%d, Temperature:%d\n",
			bq->batt_soc,bq->batt_report_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp - 2730);
	}
